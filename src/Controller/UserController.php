...
//     */
//    public function indexUserJeunes(ActualiteRepository $actualiteRepository,SpecialiteRepository $specialiteRepository,UserRepository $userRepository)
//    {  $users=$this->userRepository->findAll();
...
class UserController extends BaseController
{
    ...
    public function editUser(User $user, Request $request, TranslatorInterface $translator)
    {
        $form = $this->createForm(EditUAdmin::class, $user, ["translator" => $translator]);
...
/////////////////////////////////// *********Simple User********///////////////
    ...
    /**
     * @Route("/changevaliditecv/{id}",name="changevalidite_cv",methods={"post"})
     * @IsGranted("ROLE_SUPERUSER")
     */
    public function activatecv(User $user, UserRepository $userRepository)
    {
        $entityManager = $this->getDoctrine()->getManager();
        $user = $userRepository->changeValiditecv($user);
        $entityManager->persist($user);
        $entityManager->flush();
        return $this->json(["message" => "success", "value" => $user->getValidcv()]);
    }
